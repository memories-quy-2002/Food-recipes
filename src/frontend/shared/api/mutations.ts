...
const findOptionalCatalogId = (items: CatalogItemLike[], name: string | null | undefined): number | undefined => {
...
const toOptionalMinutes = (time: RecipeTimeInput | null | undefined): number | undefined => {
  try {
...
export const serializeWishlistPayload = (recipeId: number | string): { recipeId: number | string } => ({ recipeId })
...
const normalizeMetadataSource = (source: MetadataSource | null | undefined): MetadataSource =>
  source || 'provided_by_author'

export const serializeRecipeMetadata = (recipe: RecipeMutationForm): RecipeMetadataPayload | undefined => {
