		...
		<div className="min-w-0" aria-busy={isFetching}>
			<div className="mb-5 flex flex-col gap-4 rounded-xl border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
				...
				<div className="flex flex-wrap items-center gap-2">
					...
					<div className="flex rounded-lg border border-border p-1" aria-label="Recipe view mode"><Button variant={viewMode === "grid" ? "secondary" : "ghost"} size="icon" className="size-11" onClick={() => setViewMode("grid")} aria-pressed={viewMode === "grid"} aria-label="Grid view"><BsGrid3X3Gap /></Button><Button variant={viewMode === "list" ? "secondary" : "ghost"} size="icon" className="size-11" onClick={() => setViewMode("list")} aria-pressed={viewMode === "list"} aria-label="List view"><BsListUl /></Button></div>
