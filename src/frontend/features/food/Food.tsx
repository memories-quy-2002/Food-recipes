				...
				{!filtersReady ? <PageState type={filtersError ? "error" : undefined} title={filtersError ? "Recipe filters could not load" : "Loading recipe filters"} message={filtersError || "Fetching categories and meal filters."} /> : (
					<div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[304px_minmax(0,1fr)]">
						...
						<section className="min-w-0">
							...
							<div className="mb-4 flex items-center justify-between gap-3 lg:hidden"><Button variant="outline" className="w-full justify-center sm:w-auto" onClick={() => setIsFilterSheetOpen(true)} aria-haspopup="dialog" aria-controls="food-filter-sheet"><SlidersHorizontal className="size-4" />Filters{activeFilterCount ? ` (${activeFilterCount})` : ""}</Button></div>
