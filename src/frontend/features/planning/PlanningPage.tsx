...
const PlanningPage = () => {
	...
	const handleMoveMeal = (item: MealPlanItem, input: Pick<AddMealPlanItemInput, "date" | "slot">) => { if (!activePlan) return; updateMealMutation.mutate({ planId: activePlan.plan_id, itemId: item.item_id, input }); };
	...
	return <main className="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8 lg:py-10" aria-labelledby="planning-title">
		...
		<header className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-3xl"><h1 id="planning-title" className="text-4xl font-black tracking-[-0.035em] sm:text-5xl lg:text-6xl">Plan with intention</h1><p className="sr-only">Put recipes on the days you actually want to cook them, set servings once, and make the week easier to execute.</p></div><div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto"><Button className="flex-1 lg:flex-none" onClick={() => setIsGenerateDialogOpen(true)}><Sparkles className="size-4" aria-hidden="true" />Generate week</Button><Button asChild variant="outline" className="flex-1 lg:flex-none"><Link to="/wishlist"><Bookmark className="size-4" aria-hidden="true" />Saved recipes</Link></Button><Button asChild className="flex-1 lg:flex-none"><Link to="/shopping-list"><ShoppingBasket className="size-4" aria-hidden="true" />Shopping list</Link></Button></div></header>
