...
const PlanningPage = () => {
	...
	const createPlan = () => {
		if (canEdit) createPlanMutation.mutate({ name: "This week", from: visibleWeek.from, to: visibleWeek.to });
	};

	const handleMealSubmit = (input: AddMealPlanItemInput, itemId?: number) => {
		if (!canEdit || !activePlan) return;
		if (itemId) {
			updateMealMutation.mutate({ planId: activePlan.plan_id, itemId, input }, { onSuccess: () => setDialogState(null) });
			return;
		}
	...
	const handleRemoveMeal = (item: MealPlanItem) => {
		if (!canEdit || !activePlan || !window.confirm(`Remove ${item.recipe_name} from your plan?`)) return;
		deleteMealMutation.mutate({ planId: activePlan.plan_id, itemId: item.item_id });
	};

	const handleMoveMeal = (item: MealPlanItem, input: Pick<AddMealPlanItemInput, "date" | "slot">) => {
		if (!canEdit || !activePlan) return;
		updateMealMutation.mutate({ planId: activePlan.plan_id, itemId: item.item_id, input });
	};

	const renderPlanContent = () => {
		if (mealPlanQuery.isPending) {
			return (
				<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-7" role="status" aria-label="Loading your meal plan">
					{Array.from({ length: 7 }, (_, index) => <div key={index} className="h-64 animate-pulse rounded-2xl bg-muted" />)}
				</div>
			);
		}
		...
		if (mealPlanQuery.isError) {
			return (
				<div className="rounded-2xl border border-destructive/25 bg-destructive/10 p-6 text-center" role="alert">
					<h2 className="text-xl font-bold">Your plan could not load</h2>
					<p className="mt-2 text-muted-foreground">We could not fetch this week. Try again to keep your plan in sync.</p>
					<Button className="mt-4" onClick={() => mealPlanQuery.refetch()}>Try again</Button>
				</div>
			);
		}

		if (mealPlanQuery.data === null) {
			return (
				<div className="rounded-2xl border border-dashed border-border bg-muted/35 p-8 text-center">
					<CalendarDays className="mx-auto size-10 text-primary" />
					<h2 className="mt-4 text-2xl font-black">Plan your week</h2>
					<p className="mx-auto mt-2 max-w-xl leading-7 text-muted-foreground">Add breakfast, lunch, dinner, or snacks and turn saved recipes into a practical cooking schedule.</p>
					{canEdit ? <Button className="mt-5" onClick={createPlan} disabled={createPlanMutation.isPending} aria-busy={createPlanMutation.isPending}>{createPlanMutation.isPending ? "Creating plan…" : "Start a weekly plan"}</Button> : <p className="mt-5 text-sm font-semibold text-muted-foreground">You have read-only access to this kitchen.</p>}
					{createPlanMutation.isError && <p className="mt-3 text-sm font-semibold text-destructive" role="alert">We could not create this plan. Try again.</p>}
				</div>
			);
		}
		...
		return (
			<>
				<div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
					<p className="text-sm font-bold text-muted-foreground">View your week as a calendar or a focused agenda.</p>
					<div className="inline-flex w-full rounded-xl border border-border bg-muted/35 p-1 sm:w-auto" role="group" aria-label="Planning views">
						<button type="button" aria-pressed={planningView === "calendar"} className={`inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg px-3 text-sm font-black transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex-none ${planningView === "calendar" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:bg-background/70 hover:text-foreground"}`} onClick={() => setPlanningView("calendar")}>
							<CalendarDays className="size-4" aria-hidden="true" />Calendar
						</button>
						<button type="button" aria-pressed={planningView === "agenda"} className={`inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg px-3 text-sm font-black transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex-none ${planningView === "agenda" ? "bg-background text-foreground" : "text-muted-foreground hover:bg-background/70 hover:text-foreground"}`} onClick={() => setPlanningView("agenda")}>
							<List className="size-4" aria-hidden="true" />Agenda
						</button>
					</div>
				</div>
				<div id="planning-view-panel" className="mt-4" role="region" aria-live="polite" aria-label={`${planningView === "calendar" ? "Calendar" : "Agenda"} meal plan view`}>
					{planningView === "calendar" ? (
						<MealPlanGrid days={visibleWeek.days} items={mealPlanQuery.data?.items ?? []} onAdd={(date, slot) => setDialogState({ date, slot })} onEdit={(item) => setDialogState({ date: item.planned_date.slice(0, 10), slot: item.slot, item })} onRemove={handleRemoveMeal} onMove={handleMoveMeal} isRemoving={deleteMealMutation.isPending} readOnly={!canEdit} />
					) : (
						<MealPlanAgenda days={visibleWeek.days} items={mealPlanQuery.data?.items ?? []} onAdd={(date, slot) => setDialogState({ date, slot })} onEdit={(item) => setDialogState({ date: item.planned_date.slice(0, 10), slot: item.slot, item })} onRemove={handleRemoveMeal} onMove={handleMoveMeal} isRemoving={deleteMealMutation.isPending} readOnly={!canEdit} />
					)}
				</div>
			</>
		);
	...
	return (
		<main className="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8 lg:py-10" aria-labelledby="planning-title">
			...
			<div className="mx-auto w-full max-w-[96rem]">
				<header className="mb-7 flex flex-col gap-5 lg:flex-col lg:items-end lg:justify-between xl:flex-row">
					<div className="max-w-3xl">
						<p className="mb-2 text-sm font-bold uppercase tracking-[0.16em] text-primary">{scopeLabel}</p>
						<h1 id="planning-title" className="text-4xl font-black tracking-[-0.035em] sm:text-5xl lg:text-6xl">Plan with intention</h1>
						<p className="sr-only">Put recipes on the days you actually want to cook them, set servings once, and make the week easier to execute.</p>
					</div>
