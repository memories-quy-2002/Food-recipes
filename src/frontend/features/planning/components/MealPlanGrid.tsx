...
const MealPlanGrid = ({
	...
	const itemsByKey = new Map(
		items.map((item) => [`${item.planned_date.slice(0, 10)}:${item.slot}`, item]),
	);

	return (
		<section className="planning-grid" aria-label="Weekly meal plan">
			{days.map((day) => (
				<section className="planning-grid__day" key={day.date} aria-labelledby={`planning-day-${day.date}`}>
					<header className="planning-grid__day-header">
						<h3 id={`planning-day-${day.date}`}>{day.label.split(",")[0]}</h3>
						<span>{day.label.split(", ")[1]}</span>
					</header>
					<div className="planning-grid__slots">
						{MEAL_SLOTS.map((slot) => (
							<MealSlotComponent
								key={`${day.date}-${slot}`}
								day={day}
								slot={slot}
								item={itemsByKey.get(`${day.date}:${slot}`)}
								onAdd={onAdd}
								onEdit={onEdit}
								onRemove={onRemove}
