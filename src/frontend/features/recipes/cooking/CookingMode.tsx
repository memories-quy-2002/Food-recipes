...
const CookingMode = ({
	...
	const completeWithAction = async (action?: CookingCompletionAction): Promise<void> => {
		if (!canComplete || !isSessionReady || !isLastStep || isSaving) return;
		if (!onComplete) {
			if (planningContext || keepOpenAfterComplete) setIsComplete(true);
			else onExit();
			return;
		}
		setSaveError(null);
