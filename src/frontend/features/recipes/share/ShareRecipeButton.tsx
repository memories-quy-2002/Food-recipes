...
const ShareRecipeButton = ({ recipeId, recipeName, description, className }: ShareRecipeButtonProps): React.ReactElement => {
	...
	return (
		<>
			<Button type="button" size="icon" variant="outline" className={`size-11 ${className || ""}`} style={{ fontSize: 0 }} onClick={handleShare} disabled={isPending} aria-busy={isPending} aria-label="Share recipe" title="Share recipe">
