...
import { Pagination, Row } from "@/shared/ui/legacy-ui";
				...
				{numberReviews === 0 ? (
					...
					<ul className="recipe__content__reviews__list">
						{currentReviews.map((review) => (
							<li key={review.rating_id} className="recipe__content__reviews__list__item">
								<div className="recipe__content__reviews__list__item__container">
									<div className="recipe__content__reviews__list__item__container__context">
										<div>
											{convertImage("avatar", "recipe__content__reviews__list__item__container__context__img")}
									...
									<div className="recipe__content__reviews__list__item__container__info">
										...
										<div>
											<time dateTime={review.date_added || undefined} style={{ fontSize: "12px" }}>
				...
				<Pagination className="food__content__section__pagination">
					{totalPages <= 5 ? (
						pageNumbers.map((number) => (
							<Pagination.Item key={number} active={number === currentPage} onClick={() => handlePagination(number)}>
								{number}
							</Pagination.Item>
						))
					) : (
						<>
							...
							<Pagination.Prev onClick={() => handlePagination(currentPage > 1 ? currentPage - 1 : 1)} />
							{displayedPages.map((number) => (
								<Pagination.Item key={number} active={number === currentPage} onClick={() => handlePagination(number)}>
									{number}
								</Pagination.Item>
							))}
							<Pagination.Next onClick={() => handlePagination(currentPage < totalPages ? currentPage + 1 : totalPages)} />
