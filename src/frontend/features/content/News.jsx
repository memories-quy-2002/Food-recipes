			...
			<section className="mx-auto mt-12 grid max-w-6xl gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(260px,0.55fr)] lg:items-start">
				<div className="grid gap-4">
					{newsItems.map((item) => (
						<article className="rounded-[22px] border border-[#e7e1d9] bg-white p-6 shadow-[0_18px_40px_rgba(33,24,19,0.07)] sm:p-7" key={item.title}>
							<div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-extrabold uppercase tracking-[0.08em] text-[#9a3412]">
						...
						<Link className="mt-5 inline-flex text-sm font-extrabold text-[#8a2f0f] underline decoration-[#f0c7a1] underline-offset-4 transition-colors hover:text-[#9a3412]" to="/food">Browse recipes</Link>
						</article>
					))}
				</div>
				...
				<aside className="rounded-[22px] border border-[#e7e1d9] bg-[#f6f1e8] p-6 sm:p-7" aria-labelledby="quick-tips-title">
					...
					<ul className="mt-5 grid gap-4 text-sm leading-6 text-[#746b63]">
						{quickTips.map((tip) => (
							<li className="flex gap-3" key={tip}><span aria-hidden="true" className="mt-2 h-2 w-2 shrink-0 rounded-full bg-[#9a3412]" />{tip}</li>
						))}
					</ul>
				</aside>
			</section>
