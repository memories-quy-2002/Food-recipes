...
export const normalizeIngredient = (value) => (typeof value === "string" ? value.trim().replace(/\s+/g, " ") : "");

export const parseIngredientInput = (value) => (typeof value === "string"
	? value.split(",").map(normalizeIngredient).filter(Boolean)
	: []);

export const addIngredientToken = (current, value) => {
				...
				<div>
					<form role="search" aria-labelledby="pantry-match-title" onSubmit={handleSubmit} className="grid gap-3">
						...
						<div className="flex flex-col gap-2 sm:flex-row">
							<Input id="pantry-ingredient-input" type="text" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleInputKeyDown} placeholder="e.g. chickpeas, lemon, spinach" aria-describedby="pantry-input-help pantry-validation-message" aria-invalid={Boolean(validationMessage)} maxLength={120} />
