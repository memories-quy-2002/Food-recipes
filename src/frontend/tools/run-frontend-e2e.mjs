...
const localBuildDirectory = ".temp/playwright-managed-build";
const apiOrigin = process.env.FOOD_RECIPES_E2E_API_ORIGIN || "http://127.0.0.1:3000";
...
try {
	...
	if (!process.env.CI) previewArguments.push("--outDir", localBuildDirectory);
	...
	const test = await runProcess(
		...
		{ env: { FOOD_RECIPES_E2E_MANAGED_SERVER: "1" } },
	);
