...
export class CookingSessionService {
  ...
  async start(userId: number, dto: StartCookingSessionDto): Promise<{ session: CookingSessionRecord }> {
    ...
    const leftoverContext = dto.sourceType === 'leftover'
      ? await this.repository.leftoverStartContext!(userId, dto.recipeId, dto.leftoverBatchId!, dto.mealPlanItemId ?? null)
      : null;
    if (dto.sourceType === 'leftover' && !leftoverContext) throw new NotFoundException({ code: 'LEFTOVER_SOURCE_INVALID', message: 'Leftover source does not belong to this user or meal-plan item' });
    ...
    if (dto.sourceType !== 'leftover' && dto.mealPlanItemId !== undefined && !(await this.repository.mealPlanItemBelongsToUser(userId, dto.mealPlanItemId, dto.recipeId))) throw this.planItemNotFound();
