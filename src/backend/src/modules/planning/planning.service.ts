...
export class PlanningService {
  ...
  async addLeftoverPlanItem(userId: number, planId: number, dto: AddLeftoverMealPlanItemDto) {
    ...
    if (!item)
      throw new NotFoundException({
        code: 'LEFTOVER_NOT_AVAILABLE',
        message: 'Leftover batch is not available for this plan',
      })
    return { item }
  }
  ...
  async updatePlanItem(userId: number, planId: number, itemId: number, dto: UpdateMealPlanItemDto) {
    ...
    if (dto.recipeId && !(await this.repository.recipeExists(dto.recipeId)))
      throw new NotFoundException({ code: 'RECIPE_NOT_FOUND', message: 'Recipe not found' })
    const item = await this.repository.updatePlanItem(
      userId,
      planId,
      itemId,
      dto.recipeId,
  ...
  async deletePlanItem(userId: number, planId: number, itemId: number) {
    await this.requirePlan(userId, planId)
    if (!(await this.repository.deletePlanItemAndRecordRemoval(userId, planId, itemId)))
      throw new NotFoundException({ code: 'MEAL_PLAN_ITEM_NOT_FOUND', message: 'Meal plan item not found' })
    return { message: 'Meal plan item removed' }
  }

  listShoppingList(userId: number) {
    return this.repository.listShoppingItems(userId).then((items) => ({ items }))
  }

  async addShoppingItem(userId: number, dto: AddShoppingListItemDto) {
    if (dto.sourceRecipeId && !(await this.repository.recipeExists(dto.sourceRecipeId)))
      throw new NotFoundException({ code: 'RECIPE_NOT_FOUND', message: 'Recipe not found' })
    const item = await this.repository.addShoppingItem(
      userId,
      this.normalizeName(dto.label),
      dto.quantity?.trim() || null,
      dto.sourceRecipeId ?? null,
    )
    return { item }
  }
  ...
  async updateShoppingItem(userId: number, itemId: number, dto: UpdateShoppingListItemDto) {
    ...
    try {
      const item = await this.repository.updateShoppingItem(
        userId,
        itemId,
        label,
        dto.quantity === undefined ? undefined : dto.quantity.trim() || null,
        dto.checked,
      )
      if (!item)
        throw new NotFoundException({ code: 'SHOPPING_ITEM_NOT_FOUND', message: 'Shopping list item not found' })
      return { item }
    } catch (error) {
      if (!this.isUniqueViolation(error)) throw error
      throw new ConflictException({
        code: 'SHOPPING_ITEM_DUPLICATE',
        message: 'An unchecked shopping item with the same name and quantity already exists',
      })
    }
  }

  async deleteShoppingItem(userId: number, itemId: number) {
    if (!(await this.repository.deleteShoppingItem(userId, itemId)))
      throw new NotFoundException({ code: 'SHOPPING_ITEM_NOT_FOUND', message: 'Shopping list item not found' })
  ...
  async addRecipeIngredientsFromRecipes(userId: number, recipeIds: number[]) {
    ...
    for (const ingredient of this.consolidateStructuredIngredients(structured)) {
      items.push(
        await this.repository.addShoppingItem(
          userId,
          ingredient.name,
          this.formatStructuredQuantity(ingredient),
  ...
  private normalizeName(value: string): string {
    ...
    if (!normalized)
      throw new BadRequestException({ code: 'PLANNING_NAME_EMPTY', message: 'Name or label cannot be empty' })
    return normalized
  }
  ...
  private validateDateInRange(value: string, from: Date | string, to: Date | string) {
    ...
    if (date < this.parseDate(this.dateText(from)).getTime() || date > this.parseDate(this.dateText(to)).getTime())
      throw new BadRequestException({
        code: 'MEAL_PLAN_DATE_OUT_OF_RANGE',
        message: 'Planned date must be inside the meal plan range',
      })
  }
  ...
  private invalidRange(): BadRequestException {
    return new BadRequestException({
      code: 'MEAL_PLAN_DATE_RANGE_INVALID',
      message: 'Meal plan dates must be an inclusive range of 1 to 31 days',
    })
  }
  private planNotFound(): NotFoundException {
    return new NotFoundException({ code: 'MEAL_PLAN_NOT_FOUND', message: 'Meal plan not found' })
  }
  ...
  private isUniqueViolation(error: unknown): boolean {
    ...
    return (
      ...
      (typeof candidate.meta?.message === 'string' && candidate.meta.message.includes('23505'))
    )
  }

  private consolidateStructuredIngredients(
    ingredients: Array<StructuredShoppingIngredient & { sourceRecipeId: number }>,
  ) {
    ...
    for (const ingredient of ingredients) {
      ...
      const index = consolidated.findIndex(
        (candidate) =>
          ...
          candidate.note === ingredient.note &&
      ...
      if (index === -1) consolidated.push({ ...ingredient })
        ...
        consolidated[index] = {
          ...consolidated[index],
          quantity: Number((consolidated[index].quantity! + ingredient.quantity).toFixed(2)),
        }
    }
    return consolidated
  }
