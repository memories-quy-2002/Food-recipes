...
export class LeftoversService {
  ...
  async list(userId: number) {
    return { items: await this.repository.listAvailable(userId) };
  }

  async create(userId: number, dto: CreateLeftoverDto) {
    ...
    const existing = await this.repository.findByHistory(dto.cookingHistoryId, userId);
    const leftover = existing ?? await this.repository.create(userId, history.history_id, history.recipe_id, history.servings, dto.servings, expiresAt);
