...
| Recovery and verification | API endpoints exist for generic password recovery, single-use reset, email verification, and authenticated resend. The frontend screens are delivered in P0 B. | `src/backend/src/modules/auth/auth.controller.ts`, `features/auth` | API tests now; browser journey after P0 B |
