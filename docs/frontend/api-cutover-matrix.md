...
| Recipe detail and cooking | `features/recipes/**` | `/recipes/:id`, metadata, ratings, reviews, cooking session | Implemented; validated public metadata and Recipe JSON-LD, with bounded related-recipe reads |
...
| Shopping and pantry | `features/shopping/**`, `pantry/**` | /users/me/shopping-list and /users/me/pantry | Personal routes; duplicate-safe, retryable, and covered by the kitchen-loop regression |
| Leftovers | `features/leftovers/**` | /users/me/leftovers | Personal owner-scoped API; legacy group records are filtered out |
...
- Personal ownership, validation, and published visibility
...
5. Run recipe -> planning -> shopping -> pantry -> cooking -> history/journal,
   including personal ownership boundaries and shortage handling.
